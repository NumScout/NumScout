...
class SourceMap:
    ...
    def __init__(self, cname, parent_filename, input_type, root_path=""):
        ...
        ###
        # self.func_name_to_return_params = self._get_func_name_to_return_params()
        #####
        # self.func_call_to_params = self._get_func_call_to_params()
    ...
    ### 
    def _get_func_name_to_return_params(self):
        func_name_to_return_params = SourceMap.ast_helper.get_func_name_to_return_params(self.cname)
        for func_name in func_name_to_return_params:
            returndataload_position = 0
            for param in func_name_to_return_params[func_name]:
                if param["type"] == "ArrayTypeName":
                    param["position"] = returndataload_position
                    returndataload_position += param["value"]
                else:
                    param["position"] = returndataload_position
                    returndataload_position += 1

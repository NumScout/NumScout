...
def main():
    ...
    parser.add_argument(
        "-sv", 
        "--solc-version", 
        help="Specify solc compiler version", 
        action="store", 
        dest="solc_version",
        type=str,
    )
    ...
    parser.add_argument(
        "-pf",
        "--pruning-file",
        help="Function pruning result file",
        action="store",
        dest="pruning_file",
        type=str,
    )
    ...
    cmd = "solc-select use %s" % global_params.SOLC_VERSION
    run_command(cmd)

    if args.pruning_file:
        global_params.PRUNING_FILE = args.pruning_file

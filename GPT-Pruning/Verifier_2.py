...
class Verifier_2:
    ...
    def run(self, contract_name: str, unrelated_functions: list, code: str, sol_file_path: str, res_file_path: str):
...
2. You have to check whether the function and its calling functions actually change the amount of tokens or ethers directly.
3. If you are sure that the provided function will directly change the amount of tokens or ethers, and not indirectly or without any effect, it means that your partner's classification is wrong, otherwise it is correct.
4. Functions that indirectly affect the amount of tokens or ethers are unrelated functions and need to be classified as "correct". For example, setting or getting transaction-related variables does not directly cause changes in account balances, so it is considered an unrelated function.

# Constraints
1. Your response has to be in this JSON format without any other content: {{"correct": [...], "wrong": [...]}}.
2. Do not add functions that are not in the provided unrelated public function list. Do not omit or have overlapping functions in the two parts.

The list of unrelated public abi function of the target contract "{contract_name}" provided by your partner is as follows: {unrelated_functions}
...
"""
        ...
        if analysis_res != "error":
            ...
            print(f"已完成对文件 '{sol_file_path}' 的unrelated验证。")
        else:
